...
def get_admin_data(app, client, db_name, coll_name):
    ...
    try:
        ...
        if keys and values and coll_name not in ["department_data","attendance_data","class_data","subject_data","subject_mapping"]:
            keys = keys[0]
            for num, keys_data in enumerate(keys):
                if keys_data=="password":
                    password_pos=num
                    keys.pop(num)
            keys = [keys]
            ...
            if password_pos!=0:
                ...
                for value_data in values:
                    value_data.pop(password_pos)
                    new_values.append(value_data)
